...
function pmAnalyze(img){
  ...
  const confidence=Math.max(.38,Math.min(.88,.44+(Math.min(1,box.area/(w*h*.45))*.16)+(anis*.13)+(raised.length?Math.min(.14,raised.length*.01):0)+(borderScore>.13?.08:0)));
  return{shape,structure,base,texture,border,confidence,box:{...box,wCanvas:w,hCanvas:h},raised,cells,rows:detailRows,cols:detailCols};
}
...
function pmBuildDraft(r){
  ...
  for(let rr=0;rr<rows;rr++){
    ...
    for(let cc=0;cc<cols;cc++){
      ...
      out.push({id:makeId(),type,row:rr,col:cc,x,y,rotation:type===baseType?0:0,direction:reverse?'w':'e',photoDraft:true});
    }
  }
