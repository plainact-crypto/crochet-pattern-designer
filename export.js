...
function buildEnglishWrittenPattern(){
  ...
  const lines=[];
  const title=isRound?'Crochet Pattern - Round Construction':'Crochet Pattern - Row Construction';
  lines.push(title,'');
  ...
  lines.push('','Notes:','- This is an editable reconstructed pattern generated from the current chart.','- Stitch counts marked as estimated should be reviewed before crocheting.');
  return lines.join('\n');
}
async function exportPatternPdf(){
  ...
  doc.setFontSize(11);
  const text=buildEnglishWrittenPattern();
