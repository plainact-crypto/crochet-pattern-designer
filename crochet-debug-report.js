...
(()=>{
  ...
  function boardItems(){
    try{if(typeof window.getCrochetBoardItems==='function'){const v=window.getCrochetBoardItems();if(Array.isArray(v))return v}}catch{}
    try{if(typeof items!=='undefined'&&Array.isArray(items))return items}catch{}
    return [];
  }
  function graphState(){
    try{if(typeof window.getActiveCrochetGraph==='function'){const g=window.getActiveCrochetGraph();if(g)return g}}catch{}
    return window.activeCrochetGraph||window.lastGeneratedCrochetGraph||null;
  }
  function build(){
    const all=boardItems();
    const gen=all.filter(i=>i?.generatedPattern&&i.patternKind==='lace-flower');
    const liveGraph=graphState();
    const graph=liveGraph||{kind:'lace-flower',version:'board-reconstructed',params:{petals:5},nodes:gen,spaces:[]};
    const graphSource=liveGraph?'active-graph':'board-reconstructed';
    ...
    const orphan=[];const ids=new Set((graph?.nodes||gen).map(n=>n.id)),spaces=new Set((graph?.spaces||[]).map(s=>s.id));
    if(liveGraph){for(const n of graph.nodes||[]){if(n.workedInto&&!ids.has(n.workedInto)&&!spaces.has(n.workedInto))orphan.push({id:n.id,role:n.role,problem:'workedInto',target:n.workedInto});if(n.anchor&&!ids.has(n.anchor))orphan.push({id:n.id,role:n.role,problem:'anchor',target:n.anchor})}}
    ...
    for(let s=1;s<=5;s++){
      ...
      for(const role of roles){const arr=allSector.filter(i=>i.role===role);layers[role]={count:arr.length,bbox:bbox(arr),radial:(cx!=null?radialStats(arr,cx,cy):null),types:countBy(arr,'type'),sample:arr.slice(0,4).map(i=>({id:i.id,type:i.type,grid:[i.gridCol,i.gridRow],rotation:round2(i.rotation),workedInto:i.workedInto||null}))};}
    ...
    const warnings=[];
    if(!liveGraph)warnings.push('ACTIVE GRAPH NOT EXPOSED: topology references reconstructed from board only');
    if(!gen.length)warnings.push('NO GENERATED BOARD NODES FOUND');
    ...
    const report={
      generatedAt:new Date().toISOString(),graphSource,
      engine:{kind:graph?.kind||null,version:graph?.version||null,params:graph?.params||null,validation:graph?.validation||null},
      renderer:{name:'lace-flower-renderer-v4',gridCellPx:CELL,boardDataset:{laceFlowerV4Valid:document.getElementById('board')?.dataset?.laceFlowerV4Valid||null,generatedGridValid:document.getElementById('board')?.dataset?.generatedGridValid||null}},
      totals:{allBoardItems:all.length,boardNodes:gen.length,graphNodes:graph?.nodes?.length||0,spaces:graph?.spaces?.length||0,byRound:countBy(gen,'round'),byRole:countBy(gen,'role'),byType:countBy(gen,'type')},
  ...
  function install(){
    ...
    const pre=document.createElement('pre');pre.id='crochetDebugOutput';pre.style.cssText='display:none;margin-top:10px;max-height:420px;overflow:auto;white-space:pre-wrap;background:#0b1015;color:#dce8f5;border:1px solid #46515d;border-radius:10px;padding:12px;font:11px/1.45 ui-monospace,Consolas,monospace';
